...
const Tab: React.FC<prop> = ({ title, content }) => {
  ...
  const keyDownMove = (
    e: React.KeyboardEvent<HTMLAnchorElement> | any,
    target: Element | null | undefined
  ) => {
    if (target) {
      target.querySelector("a")?.focus();
      setTabState(
        `${target.querySelector("a")?.getAttribute("aria-controls")}`
      );
    } else if (
      !target &&
  ...
  const handleKey = (e: React.KeyboardEvent<HTMLAnchorElement>) => {
    if (e.key === "ArrowRight") {
...
const TabSC: React.FC = styled.div`
  margin-bottom: 100px;
  > ul[role="tablist"] {
    display: flex;
    padding: 0;
